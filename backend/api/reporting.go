...
func runScheduledReports() {
	...
	for rows.Next() {
		...
		// FIX: Normalize DB time "10:58:00" -> "10:58"
		dbHHMM := ""
		if len(sr.ScheduledTime) >= 5 {
			dbHHMM = sr.ScheduledTime[:5]
		}

		fmt.Printf("[cron] checking id=%d | need %s | now %s IST\n", sr.ID, dbHHMM, currentHHMM)

		if dbHHMM != currentHHMM {
			continue
		}

		// Logic check for Weekly/Monthly
		if !isReportDayDue(sr.Frequency, now) {
			continue
		}
...
func isReportDayDue(freq string, now time.Time) bool {
	switch freq {
	case "Daily":
		return true
	case "Weekly":
		// Default to Monday if no day column exists
