...
// ExportExcelBytes generates the Excel report and returns it as raw bytes
// without touching the filesystem.
func ExportExcelBytes(c *CBOM) ([]byte, error) {
	// Reuse the same workbook-building logic via a temp path approach,
	// but write into a bytes.Buffer instead of a file.
	f := excelize.NewFile()
	sheet := "CBOM Report"
	f.NewSheet(sheet)
	f.DeleteSheet("Sheet1")
	...
	headers := []string{
		"Domain", "Asset Type", "TLS Version", "Cipher Suite", "Key Exchange",
		"Cert Signature Algorithm", "Key Size", "PFS Advertised", "PFS Actual",
		"PFS Note", "Cert Expiry", "Cert Days Remaining", "HNDL Exposure Days",
		"TLS Terminator", "Terminator Source", "Crypto Agility", "Quantum Status",
		"QES Score", "DES Score", "DES Confidence", "MCS Class", "MCS Confidence",
		"QMRS Score", "RBI Compliance", "CERT-In Status", "Deprecation Date",
		"Runway Days", "Runway Status", "Recommended Algorithm", "Action",
	}
	...
	for rowIdx, asset := range c.Assets {
		row := rowIdx + 2
		values := []interface{}{
			asset.Domain, asset.AssetType, asset.TLSVersion, asset.CipherSuite, asset.KeyExchange,
			asset.CertSignatureAlgorithm, asset.KeySize, asset.PFSAdvertised, asset.PFSActual,
			asset.PFSNote, asset.CertExpiry, asset.CertDaysRemaining, asset.HNDLExposureDays,
			asset.TLSTerminator, asset.TLSTerminatorSource, asset.CryptoAgilityRating, asset.QuantumStatus,
			asset.QESRaw, asset.DESRaw, asset.DESConf, asset.MCSClass, asset.MCSConf, asset.QMRS,
			asset.RBICompliance, asset.CERTInStatus, asset.DeprecationDate, asset.RunwayDays,
			asset.RunwayStatus, asset.RecommendedAlgorithm, asset.Action,
		}
		for colIdx, val := range values {
			cell, _ := excelize.CoordinatesToCellName(colIdx+1, row)
			f.SetCellValue(sheet, cell, val)
